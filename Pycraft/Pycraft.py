...
try:
    ...
    while theme == False:
        ...
        clock.tick(60)
    themeArray = [[(255,255,255), [30,30,30], (80,80,80)],[(0,0,0), [255,230,255], (223, 128, 255)]] # font, background, shapes (dark then light)
    if theme == "dark":
    ...
    def LoadingCaption(num, version):
        if num == 0:
            ...
            pygame.display.set_caption(f"Pycraft {version} | Loading") # sets caption to this
    pygame.display.set_caption(f"Pycraft | Loading (-)") # sets caption to this
    version = "21p1003-21a" # what version of pycraft are we up to:
    ...
    pygame.display.update()
    FPS = 60 # the Frames Per Seccond is set to 60
    LoadingCaption(1, version)
    ...
    LoadingCaption(2, version)
    pygame.display.update()
    cameraANGspeed = 5 # CameraANGspeed (speed of camera panning) is set to 0.5
    LoadingCaption(3, version)
    ...
    LoadingCaption(2, version)
    pygame.display.update()
    data = open(os.path.join(base_folder,("Data_Files\\SaveGameConfig.csv")), mode ='w')
    data.close()
    LoadingCaption(3, version)#
    ...
    pygame.display.update()
    global Map
    Map = pywavefront.Wavefront(os.path.join(base_folder,("Resources\\G3_Resources\\Map\\mapv8.obj")), create_materials=True, collect_faces=True) # Map v2.obj
    LoadingCaption(1, version)
...
#44
    def Start(): # this procedure creates an intro, and checks all modules are installed.
        ...
        if data.read() == "False": # is the program is on its first run...
            ...
            wallpaper = pygame.image.load(os.path.join(base_folder,("Resources\\General_Resources\\Pycraft_Background.jpg")))
    ...
    def settings(rendis, FPS, FOV, cameraANGspeed, devmode, aa, RenderFOG, FanSky, FanPart, sound, soundVOL, music, musicVOL): # defines the setting procedure
        ...
        while True: # main settings loop
                ...
                if event.type == pygame.QUIT: # if the 'x' in the corner is pressed then;
                    ...
                    if event.key == pygame.K_SPACE and devmode < 10: # if developer mode is getting enabled...
                        ...
                        if devmode >= 5 and devmode <= 9: # if devmode is getting enabled then 
                            ...
                            pygame.display.set_caption(f"Pycraft {version}: Settings | Developer mode | V: 0,00 | FPS: {int(eFPS)} aFPS: {int((aFPS/iteration))} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())}") # tells the user
                        else:# if the developer mode is not enabled then it is set to default
                            pygame.display.set_caption(f"Pycraft: {version}: Settings") # sets the display caption
            ...
            if devmode == 10 or devmode-10 == 0:
                pygame.display.set_caption(f"Pycraft: {version}: Settings | Developer mode | V: 0,0,0 | FPS: {int(eFPS)} aFPS: {int((aFPS/iteration))} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())}")
            else:
    ...
    def Credits(numOFerrors, devmode, aa): # loads the credits menu
        base_folder = os.path.dirname(__file__)
        Display = pygame.display.set_mode((width, height))# sets the GUI size
        LoadingImage = pygame.image.load(os.path.join(base_folder,("Resources\\General_Resources\\Pycraft_Short_Loading.jpg"))) # load the loading screen (ironic?)
        Display.blit(LoadingImage, (0,0)) # displays the image to the GUI
        ...
        while True: # main game loop
            ...
            for event in pygame.event.get(): # detects events, (keypresses, display interactions, mousebutton presses, ect.)
                if event.type == pygame.QUIT: # if the 'x' in the corner is pressed then;
                    Home_Screen(numOFerrors, devmode, rendis, FPS, FOV, cameraANGspeed, aa, RenderFOG, G3Dscale, FanSky, FanPart, sound, soundVOL, music, musicVOL, camera_x, camera_y, camera_z, size) # goes back to the home screen
                elif event.type == pygame.KEYDOWN: # detects keypresses
                    if event.key == pygame.K_SPACE and devmode < 10: # if developer mode is getting enabled...
                        devmode += 1 # increases the devmode value
                        if devmode >= 5 and devmode <= 9: # if devmode is getting enabled then 
    ...
    def Home_Screen(numOFerrors, devmode, rendis, FPS, FOV, cameraANGspeed, aa, RenderFOG, G3Dscale, FanSky, FanPart, sound, soundVOL, music, MusicVOL, camera_x, camera_y, camera_z, size): # creates the home screen module used after the startup
        base_folder = os.path.dirname(__file__)
        data = open(os.path.join(base_folder,("Data_Files\\data.txt")),"r+")
        if data.read() == "True":
            contIG = 0
        ...
        pygame.display.flip() # updates the display
        if devmode == 10 or devmode-10 == 0: # if developer mode is enabled
            pygame.display.set_caption(f"Pycraft: {version}: Home Screen | Developer Mode | V: 0,0,0 | FPS: {int(clock.get_fps())} aFPS: {int(clock.get_fps())} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())}") # sets the caption to developer mode for the user
        else:# if the developer mode is not enabled then it is set to default
        ...
        if devmode == 10 or devmode-10 == 0: # if developer mode is enabled
            pygame.display.set_caption(f"Pycraft: {version}: Home | Developer Mode | V: 0,0,0 | FPS: {int(clock.get_fps())} aFPS: {int(clock.get_fps())} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())}") # sets the caption to developer mode for the user
        iteration = 1
        aFPS = 0
        while True: # main game loop
            Display.fill(BackgroundCol)
            ...
            Credits_and_Change_Log = ButtonFont5.render("Credits and Change_Log", aa, FontCol) # loads the chedits font
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    try:
                        with open(os.path.join(base_folder,("Data_Files\\SaveGameConfig.csv")), mode ='w') as saveCONFIG:
                            writer = csv.writer(saveCONFIG)
                            savingARRAY = [[(camera_x, camera_y, camera_z)], [(eFPS, aFPS)], [aa], [RenderFOG], [FanSky], [FanPart], [FOV], [devmode], [rendis], [cameraANGspeed], [(sound, soundVOL)], [(music, musicVOL)]]
                            writer.writerows(savingARRAY) # writing the data rows
                            saveCONFIG.close()
                    except Exception as error:
                        errorIMG = pygame.image.load(os.path.join(base_folder,("Resources\\General_Resources\\Saving_ERROR.jpg")))
                        Display.blit(errorIMG, (399,286))
                        pygame.display.update()
                    ...
                    if event.key == pygame.K_SPACE and devmode < 10: # if developer mode is getting enabled...
                        ...
                        if devmode >= 5 and devmode <= 9: # if devmode is getting enabled then 
                            ...
                            pygame.display.set_caption(f"Pycraft: {version}: Home | Developer mode | V: 0,0,0 | FPS: {int(eFPS)} aFPS: {int((aFPS/iteration))} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())}") # tells the user
                        else:# if the developer mode is not enabled then it is set to default
            ...
            if devmode == 10 or devmode-10 == 0:
                pygame.display.set_caption(f"Pycraft: {version}: Home | Developer mode | V: 0,0,0 | FPS: {int(eFPS)} aFPS: {int((aFPS/iteration))} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())}")
            else:
            ...
            if My >= 202 and My <= 247 and Mx >= 1155:
                ...
                if mousebuttondown == True: # if the button is clicked
                    ...
                    pygame.display.flip() # updates the display
                    if devmode == 10 or devmode-10 == 0:
                        pygame.display.set_caption(f"Pycraft: {version}: Playing | Developer mode | V: 0,0,0 | FPS: {int(eFPS)} aFPS: {int((aFPS/iteration))} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())}")
                    else:
                    ...
                    Map_size = 0
                    max_Map_size = 0
                    main(rendis, FPS, FOV, cameraANGspeed, devmode, aa, RenderFOG, Display, LoadingGameImage, G3Dscale, Map, Map_box, min_v, max_v, Map_scale, Map_trans, Map_size, max_Map_size, Load3D) # loads the command that happens when clicked
            ...
            if My >= 352 and My <= 397 and Mx >= 880:
                ...
                if mousebuttondown == True:
                    Display.blit(LoadingImage, (0,0))
                    pygame.display.flip()
                    Credits(numOFerrors, devmode, aa)
            else:
                hover5 = False
            Display.fill(BackgroundCol)
    ...
    def SunModel(Sun, Sun_scale, Sun_trans, Sun_pos_x, Sun_pos_y, Sun_pos_z): # used to create the model
        ...
        glTranslatef(Sun_pos_x, Sun_pos_y, Sun_pos_z) # and translates the mesh into view (Sun_trans)
    ...
    '''def LoadMapTexture():
        base_folder = os.path.dirname(__file__)
        Texture = Image.open(os.path.join(base_folder,("Resources\\G3_Resources\\Map\\Grass 03 seamless.jpg"))
    ...
    def DrawSkyBox(camera_x, camera_y, camera_z):
        base_folder = os.path.dirname(__file__)
    ...
    def Inventory(width, height, aa, version, FPS, Map, Map_box, min_v, max_v, Map_scale, Map_trans, Map_size, max_Map_size, Load3D):
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    Load3D = False
                    main(rendis, FPS, FOV, cameraANGspeed, devmode, aa, RenderFOG, Display, LoadingGameImage, G3Dscale, Map, Map_box, min_v, max_v, Map_scale, Map_trans, Map_size, max_Map_size, Load3D)
                if event.type == pygame.MOUSEBUTTONDOWN: # if the mouse button down
                    mousebuttondown = True # mouse button down is set to True (yes)
                if event.type == pygame.MOUSEBUTTONUP: # if the mouse button is up
                    mousebuttondown = False # this variable is set to no (False)
    ...
    def main(rendis, FPS, FOV, cameraANGspeed, devmode, aa, RenderFOG, Display, LoadingGameImage, G3Dscale, Map, Map_box, min_v, max_v, Map_scale, Map_trans, Map_size, max_Map_size, Load3D): # what requirements are needed to run this function, (are specified here)
        base_folder = os.path.dirname(__file__)
        LoadingPercent = 0
        line = []
        LoadingPercent += 100
        ...
        pygame.display.flip()
        if devmode == 10 or devmode-10 == 0:
            pygame.display.set_caption(f"Pycraft: {version}: Playing | Developer mode | V: 0,0,0 | FPS: {int(clock.get_fps())} aFPS: {int(clock.get_fps())} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())}")
        else:
            pygame.display.set_caption(f"Pycraft: {version}: Playing")
        MouseUnlock = False
        ...
        pygame.event.get()
        if Load3D == True:
            '''Sun = pywavefront.Wavefront(os.path.join(base_folder,("Resources\\G3_Resources\\Sun\\Sun.obj"))), create_materials=True, collect_faces=True)
        ...
        pygame.event.get()
        if Load3D == True:
            '''PlayerModel = pywavefront.Wavefront(os.path.join(base_folder,("Resources\\G3_Resources\\Player\\Man v2.obj")), create_materials=True, collect_faces=True)
        ...
        while True:
            ...
            firstRUN += 1
            if devmode == 10 or devmode-10 == 0:
                pygame.display.set_caption(f"Pycraft: {version}: Playing | Developer mode | V: {Total_move_x, Total_move_y, Total_move_z} | FPS: {int(eFPS)} aFPS: {int((aFPS/iteration))} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())}")
            else:
                pygame.display.set_caption(f"Pycraft: {version}: Playing")
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    pygame.display.flip()
                    Home_Screen(numOFerrors, devmode, rendis, FPS, FOV, cameraANGspeed, aa, RenderFOG, G3Dscale, FanSky, FanPart, sound, soundVOL, music, musicVOL, camera_x, camera_y, camera_z, size)
                # get keypresses
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_e:
                        ...
                        Inventory(width, height, aa, version, FPS, Map, Map_box, min_v, max_v, Map_scale, Map_trans, Map_size, max_Map_size, Load3D)
            ...
            if devmode == 10 or devmode-10 == 0:
                pygame.display.set_caption(f"Pycraft: {version}: Playing | Developer mode | V: {Total_move_x, Total_move_y, Total_move_z} C: {round(camera_x,5)}, {round((camera_y-1999.407),5)}, {round(camera_z,5)} c: {collisions[0]} | FPS: {int(eFPS)} aFPS: {int((aFPS/iteration))} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())} | weather: {weather} changeIN: {round(counterForWeather/FPS)}/300")
            else:
                pygame.display.set_caption(f"Pycraft: {version}: Playing")
            #print(collisions)
            ...
            Total_move_x, Total_move_y, Total_move_z = 0,0,0
            glDisable(GL_DEPTH_TEST)
            DrawSkyBox(camera_x, camera_y, camera_z)
            glEnable(GL_DEPTH_TEST)
