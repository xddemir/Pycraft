...
try:
    ...
    def Start(FontCol, BackgroundCol, ShapeCol):
        ...
        if data.read() == "False":
            ...
            Display.fill(BackgroundCol)
            pygame.display.set_caption(f"Pycraft: {version} | Welcome")
    ...
    def settings(themeArray, FontCol, BackgroundCol, ShapeCol, rendis, FPS, FOV, cameraANGspeed, devmode, aa, RenderFOG, FanSky, FanPart, sound, soundVOL, music, musicVOL, theme):
        base_folder = os.path.dirname(__file__)
        Display = pygame.display.set_mode((width, height))
        LoadingImage = pygame.image.load(os.path.join(base_folder,("Resources\\General_Resources\\Pycraft_Short_Loading.jpg")))
        Display.blit(LoadingImage, (0,0))
        ...
        if devmode == 10 or devmode-10 == 0:
            pygame.display.set_caption(f"Pycraft: {version}: Settings | Developer Mode")
        else:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE and devmode < 10:
                        ...
                        if devmode >= 5 and devmode <= 9:
                            pygame.display.set_caption(f"Pycraft: {version}: Settings | you are: {10-devmode} steps away from being a developer")
                        elif devmode == 10:
    ...
    def Credits(FontCol, BackgroundCol, ShapeCol, numOFerrors, devmode, aa):
        base_folder = os.path.dirname(__file__)
    ...
    def Home_Screen(themeArray, FontCol, BackgroundCol, ShapeCol, numOFerrors, devmode, rendis, FPS, FOV, cameraANGspeed, aa, RenderFOG, G3Dscale, FanSky, FanPart, sound, soundVOL, music, MusicVOL, camera_x, camera_y, camera_z, size, theme): # creates the home screen module used after the startup
        ...
        counter = 0
        oldTHEME = theme
        # trys to get an internet connection
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.connect(("fast.com", 80))
        except:
            connectionQuery = False
        else:
        ...
        while True: # main game loop
            ...
            if connectionQuery == True:
                connectionText = SideFont.render("Connected", aa, (0,connectionFadeout,0))
                connectionTimeout += 1
            ...
            if connectionQuery == True and int(connectionTimeout/(aFPS/iteration)) <= 5:
                Display.blit(connectionText, (200, 690))
                if int(connectionTimeout/(aFPS/iteration))-4 >= 0:
                    connectionCounter += 1.5
                    connectionFadeout = 255-connectionCounter
                    if connectionFadeout <= 30:
                        connectionFadeout = 30
                else:
                    connectionFadeout = 255
                    connectionCounter = 0
            else:
                connectionQuery = False
                connectionFadeout = 0 
    ...
    def MapModel(Map, Map_scale, Map_trans):
    ...
    def main(FontCol, BackgroundCol, ShapeCol, rendis, FPS, FOV, cameraANGspeed, devmode, aa, RenderFOG, Display, LoadingGameImage, G3Dscale, Map, Map_box, min_v, max_v, Map_scale, Map_trans, Map_size, max_Map_size, Load3D):
        ...
        pygame.draw.lines(Display, (153,153,153), aa, line)
        Init = LoadingFont.render("Loaded Variables", aa, FontCol)
        Display.blit(Init, (100,640))
        text = LoadQuickText()
        ...
        pygame.draw.lines(Display, (60,60,60), aa, [(95, 620), (1175, 620)], 3)
        pygame.draw.lines(Display, (153,153,153), aa, line)
        Init = LoadingFont.render("Loaded Image Resources and Initiated next step", aa, FontCol)
        Display.blit(Init, (100,640))
        text = LoadQuickText()
        LoadingTextFont.render(f"{text}", aa, FontCol)
        Display.blit(TextFontRendered, (588,160))
        ...
        pygame.draw.lines(Display, (60,60,60), aa, [(95, 620), (1175, 620)], 3)
        pygame.draw.lines(Display, (153,153,153), aa, line)
        Init = LoadingFont.render("Set Captions; Beginning Resource Loading (Loading world) | 0% complete", aa, FontCol)
        Display.blit(Init, (100, 600))
        text = LoadQuickText()
        LoadingTextFont.render(f"{text}", aa, FontCol)
        Display.blit(TextFontRendered, (588,160))
        pygame.display.flip()
        def LoadingGraphicsMAP(counterFORvertex, Display, percent, LoadingPercent):
        ...
        def LoadingMapGraphics(vertex, Map_box, counterFORvertex, LoadingPercent):
            counterFORvertex += 1
            COMPLETIONpercent = (100/6612)*counterFORvertex
            Init = LoadingFont.render(f"Set Captions; Beginning Resource Loading (Loading world) | {int(COMPLETIONpercent)}% complete", aa, FontCol)
            Display.blit(LoadingGameImage, (-1,-1))
            Display.blit(Init, (100, 600))
        ...
        if Load3D == True:
            ...
            for vertex in Map.vertices:
                min_v, max_v, Map_box, counterFORvertex = LoadingMapGraphics(vertex, Map_box, counterFORvertex, LoadingPercent)
        ...
        pygame.draw.lines(Display, (60,60,60), aa, [(95, 620), (1175, 620)], 3)
        pygame.draw.lines(Display, (153,153,153), aa, line)
        Init = LoadingFont.render("Loaded Map", aa, FontCol)
        Display.blit(Init, (100, 600))
        text = LoadQuickText()
        LoadingTextFont.render(f"{text}", aa, FontCol)
        Display.blit(TextFontRendered, (588,160))
        ...
        while True:
            ...
            if devmode == 10:
                pygame.display.set_caption(f"Pycraft: {version}: Playing | Developer mode | V: {Total_move_x, Total_move_y, Total_move_z} C: {round(camera_x,3)}, {round(camera_y,3)}, {round(camera_z,3)} (c: {collisions[0]} t: {collisions[1]}) | FPS: {int(eFPS)} aFPS: {int((aFPS/iteration))} | MemUsE: {psutil.virtual_memory().percent} | CPUUsE: {str(psutil.cpu_percent())} | weather: {weather} changeIN: {round(counterForWeather/FPS)}/300 | Theme: {theme}")
            else:
                pygame.display.set_caption(f"Pycraft: {version}: Playing")
            ...
            t1 = threading.Thread(target=MapModel(Map, Map_scale, Map_trans))
            t1.start()
            t1.join()
            #depthMapFBO = ""
