...
try: # checks all modules are installed
    ...
    def crash(error, base_folder):
        try:
            ...
            MessageFont = pygame.font.Font(os.path.join(base_folder,("Fonts\\Book Antiqua.ttf")), 15) # error? Font not initialized? v.v. vague
            Clock = pygame.time.Clock()
            try:
                cutINDEX = error.index("<module 'OpenGL.error' from ")
            except:
                donothing = 0
            else:
                cutINDEX = error.find("<")
                error = error[:(cutINDEX-1)]
            while True:
